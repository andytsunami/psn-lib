	...
	protected void parse(final TagNode rootTagNode) {
		...
		final List<TagNode> trophyDetails = rootTagNode
				.findElementByAttValue("class",
						"psnTrophyTable psnTrophyTitleTable", true, false)
				.findElementByName("tbody", false)
				.getElementListByName("td", true);

		final int bronze = Integer.parseInt(trophyDetails.get(1).getText()
				.toString());
		final int silver = Integer.parseInt(trophyDetails.get(2).getText()
				.toString());
		final int gold = Integer.parseInt(trophyDetails.get(3).getText()
				.toString());
		final int platinum = Integer.parseInt(trophyDetails.get(4).getText()
				.toString());
		*/
		// @formatter:on

		// parse trophies
		final TagNode gameListingContainer = rootTagNode.findElementByAttValue(
				"class", "gamelevelListingContainer", true, false);
		@SuppressWarnings("unchecked")
		final List<TagNode> trophyItems = gameListingContainer
				.getElementListByAttValue("class", "gameLevelListItem", true,
						false);
		...
		for (final TagNode trophyItem : trophyItems) {
			...
			trophyImage = trophyItem
					.findElementByAttValue("class", "gameLevelImage", false,
							false).findElementByName("IMG", false)
					.getAttributeByName("src");
			// check if locked
			if (trophyImage.contains("/common/icon_trophy_padlock")) trophyImage = "http://webassets.scea.com/playstation/img/trophy_locksmall.png";
			else
			...
			final TagNode gameLevelInfo = trophyItem.findElementByAttValue(
					"class", "gameLevelInfo", false, false);
