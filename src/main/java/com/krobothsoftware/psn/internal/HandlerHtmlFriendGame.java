...
public final class HandlerHtmlFriendGame extends HandlerHtml implements
	...
	public HandlerHtmlFriendGame(final String psnId) {
		list = new ArrayList<PsnGameData>();
		builder = new PsnGameData.Builder(ModelType.UK_FRIEND_VERSION, psnId);
	}
		...
		case 0:
			if (startTag.equals("a")) {
				...
				builder.setTitleLinkId((str = attributes.getValue("href"))
						.substring(str.indexOf("?title=") + 7, str.indexOf("&")));
	...
	public boolean endElement(int expr, String uri, String localName,
		...
		else if (td == -1 && qLocal(qName, localName).equals("table")) throw new StopSAXException();
	...
	public void characters(int expr, char[] ch, int start, int length)
		...
		if (calledStartElement) {
			// System.out.println(new String(ch, start, length) + " " + td);
			switch (td) {
			case 7:
